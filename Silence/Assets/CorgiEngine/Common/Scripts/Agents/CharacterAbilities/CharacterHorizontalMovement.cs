...
{	
	...
	public class CharacterHorizontalMovement : CharacterAbility 
	{
		...
		{	
			...
			bool collidingLeftOrRight = _controller.State.IsCollidingLeft || _controller.State.IsCollidingRight;
			
			if ((InstantAcceleration || collidingLeftOrRight) && (_movement.CurrentState != CharacterStates.MovementStates.WallJumping))
			{
		...
		protected override void OnEnable ()
		{
			...
			if (_character.CharacterHealth != null)
			{
				targetHealth = _character.CharacterHealth;
			}

			if (targetHealth == null)
			{
				targetHealth = this.gameObject.GetComponentInParent<Health>();
			}
			
			if (_health != null)
			{
				targetHealth.OnRevive += OnRevive;
			}	
		}
