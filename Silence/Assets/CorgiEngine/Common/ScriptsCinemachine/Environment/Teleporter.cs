...
{	
	...
	public class Teleporter : ButtonActivated 
	{
		...
		public enum CameraModes { DoNothing, TeleportCamera, CinemachinePriority, InstantCinemachinePriority }
		...
		{   
			...
			if (CameraMode == CameraModes.CinemachinePriority || CameraMode == CameraModes.InstantCinemachinePriority)
			{
			...
			if (TargetRoom != null)
			{
				if (CameraMode == CameraModes.InstantCinemachinePriority)
				{
					StartCoroutine(DelayedEnterRoomCo());
					IEnumerator DelayedEnterRoomCo()
					{
						yield return null;
