Shader "Renge/RadialSegmentedHealthBarBuiltIn" {
	Properties{
		...
		_OffsetPerBar("OffsetPerBar", Vector) = (0,0,0,0)
	...
	SubShader{
		...
		Pass {
			...
			{
				float pi = 3.14159+ _OffsetPerBar.y;
				float pi2 = pi * 2;
				...
				float lineAdd = lineMul + divpisegc+ _OffsetPerBar.x;
				float lines = mul(transuvlen, abs(sin(mod(lineAdd, 2 * divpisegc) - divpisegc)));

				float sineLines = transuvlen * sin(mod(lineAdd, 2 * divpisegc) - divpisegc + _OffsetPerBar.y);
